...
Based on the results of the overall f-test, `displ` was associated with `hwy`. `drv` was associated with `hwy` after controlling `displ`. `class` was associated with `hwy` after controlling `displ` and `drv`. Since the p-values are all less than 0.05, we reject the null hypothesis that none of the three regression coefficients are useful for predicting the highway MPG, assuming we are controlling for all of the other variables. At least one of the predictor variables has a coefficient that is different from zero and is related to `hwy`.
