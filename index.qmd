...
When looking at the summary for the model, we have P-values that again supports to reject the null hypothesis. This evidence is not the strongest though with rear wheel drive cars having a p-value of 0.015376. Based on the slope, it looks like front wheel drive cars do get the better MPG on average than the other 2 car classes when holding the other variables constant, as drvf has the most positive slope value.
...
Looking at this subset, observation 25 appears to have a decent impact on intercept, displ, and all of the car class slopes. 213 and 222 have somewhat large impacts on the drive train as well. We can use the threshhold value to further confirm if these values are impactful to our slopes.
...
```{r}
library(DAAG)
press_stat <- press(log_lm)
press_rmse <- sqrt(press_stat / nrow(mpg))
summary(log_lm)
press_rmse
```
We can see that the press_rmse value and the Residual Standard Error are fairly close. This is an indication that our model is not over fit.
```{r}
...
Using subsets for the automatic model selection, the best model used formula = log(hwy) \~ displ + drv + class + year + fl. While the automatic selection said the best model used all of the variables given, some of them are likely correlated or have some type of interaction that needs to be checked. For example, generally as you have more cylinders, you will have high displacement because displacement is a sum of all the volume displaced by all cylinders.

In the model we chose, we did not use everything because we thought certain columns were not the most relevant like year and make/model or likely related like displacement and cylinders. We can see the R-squared went up, so given more time to explore, it might be worth trying models that include more variables, but we were worried about overfitting by including everything.
...
***Future Research:*** We think some additional variables that would be helpful/relevant for this would be to include things like an aerodynamic rating for the cars and a binary columns for whether a vehicle has a turbo, and another for being a hybrid. We would think these are other factors that could be used to predict the highway mpg.

More generally, we think it would be interesting to research whether or not MPG has improved with newer cars or not. In the US, it feels like cars keep getting bigger/heavier, so we would expect that MPG keeps getting worse even if engine technology has improved. We would think having larger/bigger cars would lead to having to use engines that are less fuel efficient to power them, so it would be interesting to see if that is the case.
